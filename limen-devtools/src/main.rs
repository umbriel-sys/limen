...
#[command(
    name = "limen-devtools",
    version,
    about = "Developer tools for the Limen runtime"
)]
...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::ValidateConfig {
            ...
            run_steps,
            print_health,
        } => cmd_validate_config(&config, run_steps, print_health),
        Commands::GenerateSim {
            total_frames,
            frame_length_bytes,
            pattern,
            constant_value,
        ...
        Commands::InspectModel {
            backend,
            parameters,
        } => cmd_inspect_model(&backend, &parameters),
    }
}
...
fn cmd_validate_config(path: &str, run_steps: Option<u64>, print_health: bool) -> Result<()> {
    ...
    let mut built = build_from_configuration(&configuration, &registries)
        .map_err(|e| anyhow!("build_from_configuration failed: {e}"))?;

    if let Some(steps) = run_steps {
        built.runtime_mut().open()?;
        for _ in 0..steps {
            let _ = built.runtime_mut().run_step()?;
        }
...
fn cmd_validate_config(_path: &str, _run_steps: Option<u64>, _print_health: bool) -> Result<()> {
    Err(anyhow!(
        "validate-config requires the 'with-limen' feature."
    ))
}
...
fn cmd_generate_sim(
    ...
    if frame_length_bytes == 0 {
        return Err(anyhow!("frame_length_bytes must be greater than zero"));
    }
    ...
    for frame_index in 0..total_frames {
        let line = match pattern.to_ascii_lowercase().as_str() {
            "zeros" => (0..frame_length_bytes)
                .map(|_| "0".to_string())
                .collect::<Vec<_>>()
                .join(","),
            "ramp" => (0..frame_length_bytes)
                .map(|i| ((i % 256) as u8).to_string())
                .collect::<Vec<_>>()
                .join(","),
            "constant" => (0..frame_length_bytes)
                .map(|_| constant_value.to_string())
                .collect::<Vec<_>>()
                .join(","),
            other => {
                return Err(anyhow!(format!(
                    "unsupported pattern '{other}' (use zeros|ramp|constant)"
                )))
            }
        };
        writeln!(writer, "{line}")
...
fn cmd_inspect_model(backend_name: &str, parameters: &[String]) -> Result<()> {
    ...
    for kv in parameters {
        ...
        if parts.len() != 2 {
            return Err(anyhow!(format!(
                "invalid parameter '{kv}', expected key=value"
            )));
        }
    ...
    let factory = registries
        .get_compute_backend_factory(backend_name)
        .ok_or_else(|| anyhow!(format!("backend factory not found: '{backend_name}'")))?
        .as_ref();

    let mut backend = factory
        .create_backend()
        .map_err(|e| anyhow!(format!("failed to create backend: {e}")))?;

    let configuration = ComputeBackendConfiguration {
        label: Some(format!("devtools:{backend_name}")),
        parameters: param_map,
    };
    let mut model = backend
        .load_model(&configuration, None)
        .map_err(|e| anyhow!(format!("failed to load model: {e}")))?;
    ...
    #[cfg(feature = "alloc")]
    {
        ...
        for (i, port) in metadata.inputs.iter().enumerate() {
            println!(
                "    [{i}] name={:?}, data_type={}, shape={:?}, quant={:?}",
                port.name, port.data_type, port.shape, port.quantization
            );
        }
