...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    use limen::config::{BackpressurePolicySetting, ModelRuntimeConfiguration, NamedConfiguration, RuntimeConfiguration};
    use limen_core::traits::configuration::{ComputeBackendConfiguration, OutputSinkConfiguration, PlatformBackendConfiguration, PostprocessorConfiguration, PreprocessorConfiguration, SensorStreamConfiguration};
    ...
    let sensor_stream = NamedConfiguration::<SensorStreamConfiguration> {
        ...
        configuration: SensorStreamConfiguration { label: Some("sim".to_string()), parameters: sensor_params },
    };
    let preprocessor = NamedConfiguration::<PreprocessorConfiguration> {
        factory_name: "identity".to_string(),
        configuration: PreprocessorConfiguration { label: Some("identity_pre".to_string()), parameters: pre_params },
    };
    let model = ModelRuntimeConfiguration {
        compute_backend_factory_name: "native-identity".to_string(),
        compute_backend_configuration: ComputeBackendConfiguration { label: Some("native".to_string()), parameters: model_params },
        metadata_hint: None,
    };
    let postprocessor = NamedConfiguration::<PostprocessorConfiguration> {
        factory_name: "identity".to_string(),
        configuration: PostprocessorConfiguration { label: Some("identity_post".to_string()), parameters: post_params },
    };
    let output_sink = NamedConfiguration::<OutputSinkConfiguration> {
        factory_name: "stdout".to_string(),
        configuration: OutputSinkConfiguration { label: Some("stdout_sink".to_string()), parameters: sink_params },
    };
    let platform_backend = Some(NamedConfiguration::<PlatformBackendConfiguration> {
        factory_name: "desktop".to_string(),
        configuration: PlatformBackendConfiguration { label: Some("desktop".to_string()), parameters: BTreeMap::new() },
    });

    let configuration = RuntimeConfiguration {
        sensor_stream, preprocessor, model, postprocessor, output_sink, platform_backend,
    ...
    let mut built = build_from_configuration(&configuration, &registries).map_err(|e| format!("build failed: {e}"))?;
    built.runtime_mut().run_blocking(2).map_err(|e| format!("runtime error: {e}"))?;
    Ok(())
}
