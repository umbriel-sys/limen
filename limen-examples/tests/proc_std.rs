//! Std/concurrent tests for proc-macro (limen-build) graph.
...
#[test]
fn proc_macro_graph_std_runs() {
    use crate::ProcGraphA as GraphType; // keep name short in the test

    let (src, map, snk) = make_nodes();
    let q0: Q = Default::default();
    let q1: Q = Default::default();

    let mut graph = GraphType::new(src, map, snk, q0, q1);
    ...
    <TestStdRuntime<NoopClock, NoopTelemetry, 3, 3> as LimenRuntime<
        crate::concurrent_graph::ProcGraphAStd,
        3,
        3,
