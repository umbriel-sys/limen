...
impl SimulatedSensorNoAlloc {
    pub fn new(frame_length_bytes: usize, total_frames: u64, pattern: SimulatedPattern) -> Self {
        Self {
            ...
            timestamp_step_nanoseconds: 1_000_000,
            sequence_next: SequenceNumber(0),
        }
    }
}
impl SensorSourceBorrowed for SimulatedSensorNoAlloc {
    ...
    fn read_next_into<'a>(
        ...
        if destination_buffer.len() < self.frame_length_bytes {
            return Ok(None);
        }
        match self.pattern {
            SimulatedPattern::Zeros => {
                for b in &mut destination_buffer[..self.frame_length_bytes] {
                    *b = 0;
                }
            }
            SimulatedPattern::Ramp => {
                for (i, b) in destination_buffer[..self.frame_length_bytes]
                    .iter_mut()
                    .enumerate()
                {
                    *b = (i % 256) as u8;
            ...
            SimulatedPattern::Constant(v) => {
                for b in &mut destination_buffer[..self.frame_length_bytes] {
                    *b = v;
                }
            }
        }
        let meta = SensorFrameMeta {
