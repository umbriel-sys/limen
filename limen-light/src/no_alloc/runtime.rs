...
use limen_core::errors::{RuntimeError, SensorError, ProcessingError, OutputError, InferenceError};
...
pub struct NoAllocRuntime<S, P, M, Q, O, const SENSOR_MAX_BYTES: usize, const PREPROCESSOR_MAX_BYTES: usize, const MODEL_MAX_BYTES: usize, const POSTPROCESSOR_MAX_BYTES: usize, const MAX_DIMS: usize>
where
...
impl<S, P, M, Q, O, const SENSOR_MAX_BYTES: usize, const PREPROCESSOR_MAX_BYTES: usize, const MODEL_MAX_BYTES: usize, const POSTPROCESSOR_MAX_BYTES: usize, const MAX_DIMS: usize>
NoAllocRuntime<S, P, M, Q, O, SENSOR_MAX_BYTES, PREPROCESSOR_MAX_BYTES, MODEL_MAX_BYTES, POSTPROCESSOR_MAX_BYTES, MAX_DIMS>
where
    S: SensorSourceBorrowed, P: PreprocessorNoAlloc<MAX_DIMS>, M: ModelNoAlloc<MAX_DIMS>, Q: PostprocessorNoAlloc<MAX_DIMS>, O: OutputSinkNoAlloc,
{
    ...
    pub fn close(&mut self) -> Result<(), RuntimeError> {
        self.sensor.close().map_err(|_| RuntimeError::ComponentFailureDuringClose)?;
    ...
    pub fn run_step(&mut self) -> Result<bool, RuntimeError> {
        ...
        let sensor_len_opt = match self.sensor.read_next_into(&mut self.sensor_bytes) {
            Ok(Some((n, _meta))) => { self.stats.total_sensor_samples_received += 1; Some(n) }
            Ok(None) => None,
            Err(SensorError::EndOfStream) => { self.request_stop(); None }
            Err(_) => { self.request_stop(); None }
        };

        if let Some(sensor_len) = sensor_len_opt {
            self.pre_shape.clear();
            let pre_len = match self.pre.process(&self.sensor_bytes[..sensor_len], &mut self.pre_data_type, &mut self.pre_shape, &mut self.pre_bytes) {
                Ok(n) => { self.stats.total_preprocessor_outputs_produced += 1; n }
                Err(_) => { return Ok(true); }
            };

            self.model_shape.clear();
            let model_len = match self.model.infer(BorrowedTensorView { data_type: self.pre_data_type, shape: self.pre_shape.as_slice(), buffer: &self.pre_bytes[..pre_len] }, &mut self.model_data_type, &mut self.model_shape, &mut self.model_bytes) {
                Ok(n) => { self.stats.total_model_inferences_completed += 1; n }
                Err(_) => { return Ok(true); }
            };

            self.post_shape.clear();
            let post_len = match self.post.process(BorrowedTensorView { data_type: self.model_data_type, shape: self.model_shape.as_slice(), buffer: &self.model_bytes[..model_len] }, &mut self.post_data_type, &mut self.post_shape, &mut self.post_bytes) {
                Ok(n) => { self.stats.total_postprocessor_outputs_produced += 1; n }
                Err(_) => { return Ok(true); }
            };

            let _ = self.sink.write(BorrowedTensorView { data_type: self.post_data_type, shape: self.post_shape.as_slice(), buffer: &self.post_bytes[..post_len] }).map_err(|_e| OutputError::WriteFailed);
            self.stats.total_outputs_written += 1;
            did_work = true;
        }

        if self.state == RuntimeState::Stopping { self.close()?; }
        Ok(did_work)
    }
    pub fn health(&self) -> RuntimeHealth {
        RuntimeHealth { state: self.state, statistics: self.stats.clone(), preprocessor_input_queue_length: 0, preprocessor_input_queue_capacity: 0, postprocessor_output_queue_length: 0, postprocessor_output_queue_capacity: 0 }
    }
