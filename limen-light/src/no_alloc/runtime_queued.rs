    ...
    O: OutputSinkNoAlloc,
{
    ...
    preprocessor_input_queue:
        Deque<TensorFrame<PREPROCESSOR_MAX_BYTES, MAX_DIMS>, PREPROCESSOR_QUEUE_CAPACITY>,
    postprocessor_output_queue:
        Deque<TensorFrame<POSTPROCESSOR_MAX_BYTES, MAX_DIMS>, POSTPROCESSOR_QUEUE_CAPACITY>,
...
impl<
    ...
    NoAllocQueuedRuntime<
        ...
        MAX_DIMS,
        PREPROCESSOR_QUEUE_CAPACITY,
        POSTPROCESSOR_QUEUE_CAPACITY,
    >
where
    S: SensorSourceBorrowed,
    P: PreprocessorNoAlloc<MAX_DIMS>,
    ...
    O: OutputSinkNoAlloc,
{
    pub fn new_with_policies(
        ...
        post: Q,
        sink: O,
        preprocessor_policy: BackpressurePolicyNoAlloc,
        postprocessor_policy: BackpressurePolicyNoAlloc,
    ) -> Self {
        Self {
            sensor,
            pre,
            ...
            preprocessor_input_queue: Deque::new(),
            postprocessor_output_queue: Deque::new(),
    ...
    fn push_with_backpressure<T, const CAP: usize>(
        queue: &mut Deque<T, CAP>,
    ...
    pub fn run_step(&mut self) -> Result<bool, RuntimeError> {
            ...
            Ok(Some((sensor_len, _meta))) => {
                ...
                match self.pre.process(
                    ...
                    Ok(pre_len) => {
                        ...
                        if let Ok(frame) =
                            TensorFrame::<PREPROCESSOR_MAX_BYTES, MAX_DIMS>::try_from_parts(
                                self.pre_data_type,
                                self.pre_shape.as_slice(),
                                &self.pre_bytes[..pre_len],
                            )
                        {
                            let enq = Self::push_with_backpressure(
                                &mut self.preprocessor_input_queue,
                                self.backpressure_policy_for_preprocessor_input_queue,
                                frame,
                            );
                            if !enq {
                                self.stats.total_dropped_at_preprocessor_input_queue += 1;
                            } else {
                                did_any_work = true;
                            }
                        } else {
        ...
        if let Some(pre_frame) = self.preprocessor_input_queue.pop_front() {
            ...
            match self.model.infer(
                BorrowedTensorView {
                    data_type: pre_frame.data_type,
                    shape: pre_frame.shape.as_slice(),
                    buffer: pre_frame.bytes.as_slice(),
                },
                ...
                Ok(model_len) => {
                    ...
                    match self.post.process(
                        ...
                        Ok(post_len) => {
                            ...
                            if let Ok(frame) =
                                TensorFrame::<POSTPROCESSOR_MAX_BYTES, MAX_DIMS>::try_from_parts(
                                    self.post_data_type,
                                    self.post_shape.as_slice(),
                                    &self.post_bytes[..post_len],
                                )
                            {
                                let enq = Self::push_with_backpressure(
                                    &mut self.postprocessor_output_queue,
                                    self.backpressure_policy_for_postprocessor_output_queue,
                                    frame,
                                );
                                if !enq {
                                    self.stats.total_dropped_at_postprocessor_output_queue += 1;
                                } else {
                                    did_any_work = true;
                                }
                            } else {
        ...
        if let Some(post_frame) = self.postprocessor_output_queue.pop_front() {
            let _ = self
                ...
                .write(BorrowedTensorView {
                    data_type: post_frame.data_type,
                    shape: post_frame.shape.as_slice(),
                    buffer: post_frame.bytes.as_slice(),
                })
        ...
        if self.state == RuntimeState::Stopping
            && self.preprocessor_input_queue.is_empty()
            && self.postprocessor_output_queue.is_empty()
        {
            self.close()?;
        }
