...
pub struct NativeIdentityModel {
...
impl Model for NativeIdentityModel {
    ...
    fn infer(&mut self, tensor_input: &TensorInput) -> Result<TensorOutput, InferenceError> {
        ...
        TensorOutput::new(
            tensor_input.data_type,
            tensor_input.shape.clone(),
            tensor_input.strides.clone(),
            buffer,
        )
...
impl ComputeBackend for NativeIdentityComputeBackend {
    fn load_model(
        &mut self,
        _configuration: &ComputeBackendConfiguration,
...
impl ComputeBackendFactory for NativeIdentityComputeBackendFactory {
    ...
    fn create_backend(&self) -> Result<Box<dyn ComputeBackend>, InferenceError> {
        Ok(Box::new(NativeIdentityComputeBackend::default()))
    }
}
