...
pub struct GeneralPurposeInputOutputSink {
    pin: OutputPin,
    active_high: bool,
}
...
impl OutputSinkFactory for GeneralPurposeInputOutputSinkFactory {
    ...
    fn create_output_sink(
        ...
        let pin_number: u8 = configuration
            .parameters
            .get("pin")
            .and_then(|s| s.parse::<u16>().ok())
            .and_then(|v| u8::try_from(v).ok())
            .ok_or(OutputError::Other {
                message: "missing or invalid 'pin'".to_string(),
            })?;
        let active_high: bool = configuration
            .parameters
            .get("active_high")
            .map(|s| s == "1" || s.eq_ignore_ascii_case("true"))
            .unwrap_or(true);
        let gpio = Gpio::new().map_err(|_| OutputError::Other {
