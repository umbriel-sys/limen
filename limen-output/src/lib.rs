...
pub fn register_all(
    ...
    #[cfg(all(feature = "std", feature = "stdout"))]
    {
        registries.register_output_sink_factory_with_name(
            "stdout".to_string(),
            Box::new(stdout_sink::StandardOutputSinkFactory),
        )?;
    }
    #[cfg(all(feature = "std", feature = "file"))]
    {
        registries.register_output_sink_factory_with_name(
            "file".to_string(),
            Box::new(file_sink::FileOutputSinkFactory),
        )?;
    }
    #[cfg(all(feature = "std", feature = "mqtt-paho"))]
    {
        registries.register_output_sink_factory_with_name(
            "mqtt".to_string(),
            Box::new(mqtt_paho_sink::MessageQueuingTelemetryTransportSinkFactory),
        )?;
    }
    #[cfg(all(feature = "std", feature = "gpio-rpi"))]
    {
        registries.register_output_sink_factory_with_name(
            "gpio".to_string(),
            Box::new(gpio_rpi_sink::GeneralPurposeInputOutputSinkFactory),
        )?;
    }
    Ok(())
}
