...
impl OutputSink for MessageQueuingTelemetryTransportSink {
    fn write(&mut self, output: &TensorOutput) -> Result<(), OutputError> {
        let message =
            mqtt::Message::new(&self.topic, output.buffer.clone(), self.quality_of_service);
        self.client
            .publish(message)
...
impl OutputSinkFactory for MessageQueuingTelemetryTransportSinkFactory {
    ...
    fn create_output_sink(
        ...
        let server_uri = configuration
            .parameters
            .get("server_uri")
            .ok_or(OutputError::Other {
                message: "missing 'server_uri'".to_string(),
            })?
            .to_string();
        ...
        let topic = configuration
            ...
            .to_string();
        let qos: i32 = configuration
            .parameters
            .get("qos")
            .and_then(|s| s.parse::<i32>().ok())
            .unwrap_or(0);
        let retain: bool = configuration
            .parameters
            .get("retain")
        ...
        let create_options = mqtt::CreateOptionsBuilder::new()
            ...
            .client_id(client_id)
            .finalize();
        let client = mqtt::Client::new(create_options).map_err(|_| OutputError::Other {
            message: "failed to create MQTT client".to_string(),
        })?;
        ...
        client
            ...
            .map_err(|_| OutputError::Other {
                message: "MQTT connection failed".to_string(),
            })?;
        Ok(Box::new(MessageQueuingTelemetryTransportSink {
            client,
            topic,
