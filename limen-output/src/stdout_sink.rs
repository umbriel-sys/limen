...
impl OutputSink for StandardOutputSink {
    fn write(&mut self, output: &TensorOutput) -> Result<(), OutputError> {
        ...
        let line = format!(
            ...
            output.data_type, output.shape, output.buffer.len(), preview_hex.trim_end()
        );
...
impl OutputSinkFactory for StandardOutputSinkFactory {
    fn sink_name(&self) -> &'static str { "stdout" }
    fn create_output_sink(&self, configuration: &OutputSinkConfiguration) -> Result<Box<dyn OutputSink>, OutputError> {
        let show_preview_bytes = configuration.parameters.get("preview_bytes").and_then(|s| s.parse::<usize>().ok()).unwrap_or(16);
        Ok(Box::new(StandardOutputSink { writer: BufWriter::new(std::io::stdout()), show_preview_bytes }))
    }
}
