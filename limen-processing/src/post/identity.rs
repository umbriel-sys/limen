...
impl Postprocessor for IdentityPostprocessor {
    fn process(&mut self, model_output: TensorOutput) -> Result<TensorOutput, ProcessingError> { Ok(model_output) }
...
impl PostprocessorFactory for IdentityPostprocessorFactory {
    fn postprocessor_name(&self) -> &'static str { "identity" }
    fn create_postprocessor(&self, _configuration: &PostprocessorConfiguration) -> Result<Box<dyn Postprocessor>, ProcessingError> {
