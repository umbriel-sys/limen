...
#[derive(Clone, Debug)]
pub struct ThresholdPostprocessor {
    threshold: f32,
}
    ...
    fn process(&mut self, model_output: TensorOutput) -> Result<TensorOutput, ProcessingError> {
        if model_output.data_type != DataType::Float32 {
            return Err(ProcessingError::InvalidData {
                message: "threshold postprocessor expects Float32 input".to_string(),
            });
        }
        if model_output.buffer.len() < core::mem::size_of::<f32>() {
            return Err(ProcessingError::InvalidData {
                message: "model output buffer too small for Float32 element".to_string(),
            });
        }
    ...
    fn create_postprocessor(
        ...
        #[cfg(feature = "alloc")]
        {
            if let Some(s) = configuration.parameters.get("threshold") {
                threshold_value =
                    Some(s.parse::<f32>().map_err(|_| ProcessingError::InvalidData {
                        message: "invalid 'threshold' parameter".to_string(),
                    })?);
            }
        }
        let threshold = threshold_value.ok_or_else(|| ProcessingError::InvalidData {
            message: "threshold postprocessor requires 'threshold' parameter".to_string(),
        })?;
