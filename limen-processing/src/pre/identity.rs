...
impl IdentityPreprocessor {
    fn compute_shape_for_payload_len(
        &self,
        payload_len_bytes: usize,
    ) -> Result<alloc::boxed::Box<[usize]>, ProcessingError> {
        let element_size = self.configured_data_type.byte_size();
        if element_size == 0 {
            return Err(ProcessingError::InvalidData {
                message: "data type has zero byte size".to_string(),
            });
        }
        if payload_len_bytes % element_size != 0 {
            return Err(ProcessingError::InvalidData {
                message: "payload length is not a multiple of the selected data type size"
                    .to_string(),
            });
        }
...
impl Preprocessor for IdentityPreprocessor {
    fn process(
        ...
        let shape = if let Some(s) = &self.configured_shape_override {
            s.clone()
        } else {
            self.compute_shape_for_payload_len(sensor_data.payload.len())?
        };
        let expected_len = shape
            .iter()
            .copied()
            .product::<usize>()
            .checked_mul(self.configured_data_type.byte_size())
            .ok_or(ProcessingError::InvalidData {
                message: "byte length overflow".to_string(),
            })?;
        if expected_len != sensor_data.payload.len() {
            return Err(ProcessingError::InvalidData {
                message: "payload length does not match the configured shape and data type"
                    .to_string(),
            });
        }
        let buffer: Vec<u8> = sensor_data.payload.to_vec();
        TensorInput::new(self.configured_data_type, shape, None, buffer).map_err(|e| {
    ...
    fn create_preprocessor(
        ...
        #[cfg(feature = "alloc")]
        {
            if let Some(t) = configuration.parameters.get("data_type") {
                data_type = Some(
                    parse_data_type(t).map_err(|m| ProcessingError::InvalidData { message: m })?,
                );
            }
        ...
        let data_type = data_type.ok_or_else(|| ProcessingError::InvalidData {
            message: "identity preprocessor requires 'data_type' parameter".to_string(),
        })?;

        Ok(Box::new(IdentityPreprocessor {
            configured_data_type: data_type,
            configured_shape_override: shape_override,
        }))
    }
}
