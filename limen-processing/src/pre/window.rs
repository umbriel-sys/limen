    ...
    fn validate_shape(
        ...
        if product != window_length_elements {
            return Err(ProcessingError::InvalidData {
                message: "output_shape does not multiply to window_length_elements".to_string(),
            });
        }
        Ok(())
    }
...
impl Preprocessor for WindowPreprocessor {
    fn process(
        ...
        if self.buffer.len() < self.window_length_elements {
            return Err(ProcessingError::OperationFailed {
                message: "insufficient data to produce a window".to_string(),
            });
        }
        let mut window_bytes: Vec<u8> = Vec::with_capacity(self.window_length_elements);
        for _ in 0..self.window_length_elements {
            window_bytes.push(self.buffer.pop_front().unwrap());
        }
...
impl PreprocessorFactory for WindowPreprocessorFactory {
    ...
    fn create_preprocessor(
        ...
        #[cfg(feature = "alloc")]
        {
            if let Some(s) = configuration.parameters.get("window_length_elements") {
                window_length_elements =
                    Some(
                        s.parse::<usize>()
                            .map_err(|_| ProcessingError::InvalidData {
                                message: "invalid 'window_length_elements' parameter".to_string(),
                            })?,
                    );
            }
            if let Some(s) = configuration.parameters.get("hop_length_elements") {
                hop_length_elements =
                    Some(
                        s.parse::<usize>()
                            .map_err(|_| ProcessingError::InvalidData {
                                message: "invalid 'hop_length_elements' parameter".to_string(),
                            })?,
                    );
            }
        ...
        let window_length_elements =
            window_length_elements.ok_or_else(|| ProcessingError::InvalidData {
                message: "window preprocessor requires 'window_length_elements' parameter"
                    .to_string(),
            })?;
