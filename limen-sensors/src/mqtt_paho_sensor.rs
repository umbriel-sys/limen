...
impl SensorStreamFactory for MessageQueuingTelemetryTransportSensorFactory {
    fn sensor_name(&self) -> &'static str { "mqtt" }

    fn create_sensor_stream(&self, configuration: &SensorStreamConfiguration) -> Result<Box<dyn SensorStream>, SensorError> {
        let server_uri = configuration.parameters.get("server_uri").ok_or(SensorError::ConfigurationInvalid)?.to_string();
        let client_id = configuration.parameters.get("client_id").ok_or(SensorError::ConfigurationInvalid)?.to_string();
        let topic = configuration.parameters.get("topic").ok_or(SensorError::ConfigurationInvalid)?.to_string();
        let quality_of_service: i32 = configuration.parameters.get("qos").and_then(|s| s.parse::<i32>().ok()).unwrap_or(0);
        ...
        client.connect(connect_options).map_err(|_| SensorError::OpenFailed)?;
        client.subscribe(&topic, quality_of_service).map_err(|_| SensorError::OpenFailed)?;
