...
use limen_core::types::{SensorData, SequenceNumber, SensorSampleMetadata, TimestampNanosecondsSinceUnixEpoch};
...
impl SensorStream for SerialPortSensor {
    ...
    fn read_next<'a>(&'a mut self) -> Result<Option<SensorData<'a>>, SensorError> {
        ...
        match self.port.read(&mut temporary) {
            Ok(count) => { if count > 0 { self.buffer.extend_from_slice(&temporary[..count]); } }
            Err(e) if e.kind() == ErrorKind::TimedOut || e.kind() == ErrorKind::WouldBlock => {  }
            Err(_) => return Err(SensorError::ReadFailed),
        }
        if self.buffer.len() < self.frame_length_bytes { return Ok(None); }
        let frame = self.buffer.drain(..self.frame_length_bytes).collect::<Vec<u8>>();
        let data = SensorData {
    ...
    fn reset(&mut self) -> Result<(), SensorError> { self.buffer.clear(); Ok(()) }
...
impl SensorStreamFactory for SerialPortSensorFactory {
    fn sensor_name(&self) -> &'static str { "serial" }

    fn create_sensor_stream(&self, configuration: &SensorStreamConfiguration) -> Result<Box<dyn SensorStream>, SensorError> {
        let port_name = configuration.parameters.get("port_name").ok_or(SensorError::ConfigurationInvalid)?.to_string();
        let baud_rate: u32 = configuration.parameters.get("baud_rate").and_then(|s| s.parse::<u32>().ok()).ok_or(SensorError::ConfigurationInvalid)?;
        let frame_length_bytes: usize = configuration.parameters.get("frame_length_bytes").and_then(|s| s.parse::<usize>().ok()).ok_or(SensorError::ConfigurationInvalid)?;

        let timestamp_step_nanoseconds: u128 = configuration.parameters.get("timestamp_step_nanoseconds").and_then(|s| s.parse::<u128>().ok()).unwrap_or(1_000_000);
        let sequence_start: u64 = configuration.parameters.get("sequence_start").and_then(|s| s.parse::<u64>().ok()).unwrap_or(0);
        let sample_rate_hz: Option<u32> = configuration.parameters.get("sample_rate_hz").and_then(|s| s.parse::<u32>().ok());
        let channel_count: Option<u16> = configuration.parameters.get("channel_count").and_then(|s| s.parse::<u16>().ok());

        let port = serialport::new(port_name, baud_rate).timeout(Duration::from_millis(0)).open().map_err(|_| SensorError::OpenFailed)?;
