    ...
    fn build_payload(&self) -> Vec<u8> {
        match self.pattern {
            ...
            SimulatedPattern::Ramp => {
                ...
                for i in 0..self.frame_length_bytes { buffer.push((i % 256) as u8); }
                buffer
            }
...
impl SensorStream for SimulatedSensor {
    ...
    fn read_next<'a>(&'a mut self) -> Result<Option<SensorData<'a>>, SensorError> {
        ...
        if self.frames_emitted >= self.total_frames_to_emit { return Err(SensorError::EndOfStream); }
    ...
    fn reset(&mut self) -> Result<(), SensorError> { self.frames_emitted = 0; Ok(()) }
...
impl SensorStreamFactory for SimulatedSensorFactory {
    fn sensor_name(&self) -> &'static str { "simulated" }
    fn create_sensor_stream(&self, configuration: &SensorStreamConfiguration) -> Result<Box<dyn SensorStream>, SensorError> {
        ...
        if let Some(s) = configuration.parameters.get("frame_length_bytes") { frame_length_bytes = Some(s.parse::<usize>().map_err(|_| SensorError::ConfigurationInvalid)?); }
        if let Some(s) = configuration.parameters.get("total_frames") { total_frames_to_emit = s.parse::<u64>().map_err(|_| SensorError::ConfigurationInvalid)?; }
        if let Some(s) = configuration.parameters.get("pattern") {
            ...
            match lower.as_str() {
                ...
                "constant" => {
                    let constant_value = configuration.parameters.get("constant_value").and_then(|v| v.parse::<u8>().ok()).unwrap_or(0u8);
                    pattern = SimulatedPattern::Constant(constant_value);
                }
                _ => return Err(SensorError::ConfigurationInvalid),
            }
        }
        if let Some(s) = configuration.parameters.get("timestamp_step_nanoseconds") { timestamp_step_nanoseconds = s.parse::<u128>().map_err(|_| SensorError::ConfigurationInvalid)?; }
        if let Some(s) = configuration.parameters.get("sequence_start") { sequence_start = s.parse::<u64>().map_err(|_| SensorError::ConfigurationInvalid)?; }
        if let Some(s) = configuration.parameters.get("sample_rate_hz") { sample_rate_hz = Some(s.parse::<u32>().map_err(|_| SensorError::ConfigurationInvalid)?); }
        if let Some(s) = configuration.parameters.get("channel_count") { channel_count = Some(s.parse::<u16>().map_err(|_| SensorError::ConfigurationInvalid)?); }
