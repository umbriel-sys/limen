...
use limen_core::types::{
    ModelMetadata, SensorData, SensorSampleMetadata, TensorInput, TensorOutput,
};
...
impl Preprocessor for BoxedPreprocessor {
    fn process(&mut self, sensor_data: SensorData<'_>) -> Result<TensorInput, ProcessingError> {
        self.0.process(sensor_data)
    }
