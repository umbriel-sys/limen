...
use crate::boxed::{BoxedModel, BoxedOutputSink, BoxedPostprocessor, BoxedPreprocessor, BoxedSensorStream};
...
use limen_core::traits::{ComputeBackend, Model, OutputSink, PlatformBackend, Postprocessor, Preprocessor, SensorStream};

#[derive(Debug)]
pub enum BuildError {
    FactoryNotFound { component: &'static str, factory_name: alloc::string::String },
...
impl fmt::Display for BuildError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        use BuildError::*;
        match self {
            FactoryNotFound { component, factory_name } => write!(f, "factory not found for {}: '{}'", component, factory_name),
            ComputeBackendCreationFailed(e) => write!(f, "compute backend creation failed: {:?}", e),
            ...
            PlatformBackendCreationFailed(e) => write!(f, "platform backend creation failed: {:?}", e),
            PlatformBackendInitializationFailed(e) => write!(f, "platform backend initialization failed: {:?}", e),
...
pub struct BuiltRuntime {
    pub runtime: Runtime<BoxedSensorStream, BoxedPreprocessor, BoxedModel, BoxedPostprocessor, BoxedOutputSink>,
    pub platform_backend: Option<Box<dyn PlatformBackend>>,
}

#[cfg(feature = "alloc")]
impl BuiltRuntime {
    pub fn runtime(&self) -> &Runtime<BoxedSensorStream, BoxedPreprocessor, BoxedModel, BoxedPostprocessor, BoxedOutputSink> { &self.runtime }
    pub fn runtime_mut(&mut self) -> &mut Runtime<BoxedSensorStream, BoxedPreprocessor, BoxedModel, BoxedPostprocessor, BoxedOutputSink> { &mut self.runtime }
    pub fn platform_backend(&self) -> Option<&Box<dyn PlatformBackend>> { self.platform_backend.as_ref() }
}
...
pub fn build_from_configuration(
    ...
    let sensor_factory = registries
        ...
        .ok_or_else(|| BuildError::FactoryNotFound { component: "sensor_stream", factory_name: configuration.sensor_stream.factory_name.clone() })?;
    ...
    let preprocessor_factory = registries
        ...
        .ok_or_else(|| BuildError::FactoryNotFound { component: "preprocessor", factory_name: configuration.preprocessor.factory_name.clone() })?;
    ...
    let backend_factory = registries
        ...
        .ok_or_else(|| BuildError::FactoryNotFound { component: "compute_backend", factory_name: configuration.model.compute_backend_factory_name.clone() })?;
    ...
    let postprocessor_factory = registries
        ...
        .ok_or_else(|| BuildError::FactoryNotFound { component: "postprocessor", factory_name: configuration.postprocessor.factory_name.clone() })?;
    ...
    let output_sink_factory = registries
        ...
        .ok_or_else(|| BuildError::FactoryNotFound { component: "output_sink", factory_name: configuration.output_sink.factory_name.clone() })?;
    ...
    if let Some(platform_cfg) = &configuration.platform_backend {
        let platform_factory = registries
            ...
            .ok_or_else(|| BuildError::FactoryNotFound { component: "platform_backend", factory_name: platform_cfg.factory_name.clone() })?;
    ...
    if pre_q_cap == 0 || post_q_cap == 0 {
        return Err(BuildError::InvalidConfiguration(alloc::string::String::from("queue capacities must be greater than zero")));
    }

    let pre_policy = configuration.backpressure_policy_for_preprocessor_input_queue.to_backpressure_policy();
    let post_policy = configuration.backpressure_policy_for_postprocessor_output_queue.to_backpressure_policy();
    ...
    Ok(BuiltRuntime { runtime, platform_backend })
}
